...
namespace SortedCollections
{
    ...
    class MyLinkedList_v2<T> : ILinkedList<T> where T : IComparable
    {
        ...
        public void Add(T data)
        {
            ...
            {
                // If not, set the last node's Next to our new node and update the tail reference
                tail.Next = newNode;
                tail = newNode;
            }
        ...
        public bool Remove(T data)
        {
            ...
            {
                // If count is 1, we're effectively clearing the list
                if(count == 1)
                {
                    tail = null;
                    head = null;
                }
                // Otherwise, just remove the head by changing it to refer to the second element
                else
                {
                    head = head.Next;
                }

                // Always reduce the count and return true
                count--;
                return true;
            }
            ...
            {
                // If we're about to remove the last element, update tail
